...
datenums_equities_rtns = getfield(returns_equities_ts,'dates');
...
returns_equities = fts2mat(returns_equities_ts);
prices_equities = fts2mat(prices_equities_ts);
%take logs (since we have it converted)
prices_equities = log(prices_equities);
...
equitiestime_rtns = repmat(time, size(returns_equities,1), 1);
equitiestime_prices = repmat(time, size(prices_equities,1), 1);

%add the timestamp
returns_equities = [datevec_equities_rtns(:,1:3) equitiestime_rtns ...
    returns_equities];
prices_equities = [datevec_equities_prices(:,1:3) equitiestime_prices ... 
    prices_equities];

%reconvert to time series object
prices_equities_ts = fints(datenum(prices_equities(:,1:6)),...
    prices_equities(:,7:size(prices_equities,2)), names_equities);
returns_equities_ts = fints(datenum(returns_equities(:,1:6)),...
    returns_equities(:,7:size(returns_equities,2)), names_equities);
...
returns_equities_ts = returns_equities_ts(datestr(common_datenums_all));
...
prices_diff_ts = prices_TWAP_ts - prices_11am_ts;
